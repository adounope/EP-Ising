...
folder_name = f'./results/{ep_model.N_spins}_{time_string}_MNIST_1000_step_test_b{ep_model.beta}'
...
#                               1 .. 10
N_temp_steps = [2, 3, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
for N_temp_step in N_temp_steps:
    ep_model.init_params()
    ep_model.N_neal_temps = N_temp_step
    ep_model.N_rev_neal_temps = N_temp_step
    ep_model.create_schedule()

    best_val_acc = 0
    acc_history = []
    os.system(f'mkdir {folder_name}/{N_temp_step}')
    os.system(f'mkdir {folder_name}/{N_temp_step}/weights')
    os.system(f'touch {folder_name}/{N_temp_step}/train_log.txt')
    with open(f'{folder_name}/{N_temp_step}/train_log.txt', 'a') as f:
    ...
    for epoch in tqdm(range(N_epoch), f'N_neal_temps={N_temp_step}'):
        ...
        current_best = False
        np.save(f'{folder_name}/{N_temp_step}/weights/{epoch}_J', ep_model.J)
        np.save(f'{folder_name}/{N_temp_step}/weights/{epoch}_H', ep_model.H)
        np.save(f'{folder_name}/{N_temp_step}/weights/{epoch}_W', ep_model.W)
    ...
    np.save(f'{folder_name}/{N_temp_step}/acc_history', acc_history)
    ...
    plt.ylim(0, 100)
    plt.savefig(f'{folder_name}/{N_temp_step}/acc_history.png')
    plt.cla()
